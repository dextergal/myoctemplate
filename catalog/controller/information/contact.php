<?php
	class ControllerInformationContact extends Controller {
		...
		public function index() {
			...
			$data['store_email'] = nl2br($this->config->get('config_email'));
		...
		public function enquiry() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate_enquiry()) {
				...
				$mail->setSubject(html_entity_decode(sprintf($this->language->get($this->request->post['q_subject']), $this->request->post['q_name']), ENT_QUOTES, 'UTF-8'));
		...
		protected function validate_enquiry() {
			...
			if ((utf8_strlen($this->request->post['q_name']) < 3) || (utf8_strlen($this->request->post['q_name']) > 32)) {
				$this->error['q_name'] = $this->language->get('error_q_name');
			}
			
			if ((int)$this->request->post['q_contact'] < 1) {
				$this->error['q_contact'] = $this->language->get('error_q_contact');
			}
			
			if ((utf8_strlen($this->request->post['q_subject']) < 1) || (utf8_strlen($this->request->post['q_subject']) > 32)) {
				$this->error['q_subject'] = $this->language->get('error_q_subject');
			}
	
			if (!filter_var($this->request->post['q_email'], FILTER_VALIDATE_EMAIL)) {
				$this->error['q_email'] = $this->language->get('error_q_email');
			}
			
			if ((utf8_strlen($this->request->post['q_message']) < 10) || (utf8_strlen($this->request->post['q_message']) > 3000)) {
				$this->error['q_message'] = $this->language->get('error_q_message');
			} 

			if ((utf8_strlen($this->request->post['g-recaptcha-response']) < 10) || (utf8_strlen($this->request->post['g-recaptcha-response']) > 3000)) {
				$this->error['q_captcha'] = $this->language->get('error_q_captcha');
			} 
